...
mabakhara_submenu = [
    {"label": "مبخرة موديل 1", "callback": "mabakhara_m1", "image": "https://png.pngtree.com/png-vector/20230531/ourmid/pngtree-banana-coloring-page-vector-png-image_6787674.png", "description": "مبخرة موديل 1 بتصميم راقي ومميز.", "price": "150 ج"},
    {"label": "مبخرة موديل 2", "callback": "mabakhara_m2", "image": "https://png.pngtree.com/png-vector/20230531/ourmid/pngtree-banana-coloring-page-vector-png-image_6787674.png", "description": "مبخرة موديل 2 بتصميم عصري وجذاب.", "price": "200 ج"}
]
...
main_menu = [
    ...
    {"label": "♨️ مباخر", "callback": "mabakhara"}, # 🔥 الزر الجديد (تمت الإضافة)
...
all_submenus = {
    ...
    "mabakhara": mabakhara_submenu # 🔥 تمت الإضافة
}

# بناء خريطة المنتجات (مفتاح المنتج > مفتاح القائمة الأم)
product_to_submenu_map = {}
for menu_key, submenu_list in all_submenus.items():
    if menu_key in ["bsamat", "wedding_tissues", "abajorat", "engraved_wallet", "aqlam", "katb_kitab_box", "mirrors", "fans", "sublimation", "clocks", "mabakhara"]: 
...
def show_product_page(update, product_callback_data, product_list, is_direct_list=False):
    ...
    if product_callback_data in ["bsamat", "wedding_tissues", "abajorat", "katb_kitab_box", "mirrors", "fans", "sublimation", "clocks", "mabakhara"]: 
...
def save_mirror_size_ask_name(update, context):
    ...
    # تحديد السعر واسم المقاس بناءً على الزر المضغوط
    size_price = ""
    size_label = ""
    ...
    if data == "size_1":
        ...
        # لو حدث خطأ أو ضغط زر غير متوقع
        return GET_MIRROR_SIZE

    # تحديث سعر المنتج في البيانات المحفوظة
    ...
    context.bot.send_message(
        chat_id=update.effective_chat.id,
        text=f"✅ تم اختيار **{size_label}** بسعر **{size_price}**.\n\nمن فضلك الآن **اكتب اسم العروسة المطلوب حفره على المرايا** في رسالة نصية بالأسفل، أو اضغط زر رجوع للإلغاء:",
        reply_markup=reply_markup,
        parse_mode="Markdown"
    )
...
def get_mabakhara_items():
    return mabakhara_submenu

def start_mabakhara_purchase(update, context):
    query = update.callback_query
    query.answer()
    data = query.data  # buy_mabakhara_m1
    product_callback = data.replace("buy_", "")
    
    # 1. الحصول على بيانات المنتج
    items_list = get_mabakhara_items() 
    selected_product = next((item for item in items_list if item["callback"] == product_callback), None)
    if not selected_product:
        query.answer("خطأ في العثور على المنتج", show_alert=True)
        return ConversationHandler.END
        
    context.user_data['mabakhara_product'] = selected_product
    context.user_data['state'] = GET_MABKHARA_DETAILS
    
    # 2. زر الرجوع
    back_keyboard = [[InlineKeyboardButton("🔙 رجوع", callback_data="mabakhara")]] 
    reply_markup = InlineKeyboardMarkup(back_keyboard)
    
    # 3. إرسال الرسالة والصورة
    try:
        query.message.delete()
    except:
        pass
        
    caption_text = f"✅ **{selected_product['label']}** (السعر: *{selected_product.get('price', 'غير متوفر')}*)\n\nاكتب هنا اي تفاصيل اضافية خاصة بك\nأو اكتب **لا يوجد** اذا لم يكن لديك اي تفاصيل اضافية\nاو اضغط زر رجوع للعودة الي القائمة السابقة."
...
def back_to_mabakhara_menu(update, context):
    query = update.callback_query
    query.answer()
    context.user_data.clear()
    ...
    show_product_page(update, "mabakhara", mabakhara_submenu, is_direct_list=True)
    return ConversationHandler.END

def receive_mabakhara_details_and_finish(update, context):
    details = update.message.text
    context.user_data['mabakhara_details'] = details
    
    return prompt_for_payment_and_receipt(update, context, product_type="مباخر")
...
def prompt_for_payment_and_receipt(update, context, product_type):
    ...
    if product_type == "بصامة":
        ...
        product_type = f"{product_type} - {size_label}"
    elif product_type == "مباخر": # 🔥 إضافة حالة المباخر (جديد)
        product_data = context.user_data.get('mabakhara_product')
        names_details = context.user_data.get('mabakhara_details')
...
def handle_payment_photo(update, context):
    ...
    message_body = (
        ...
        f"الأسماء (أو الحفر/التفاصيل): {names_text}\n"
        f"التاريخ: {date_text}\n"
...
def button(update, context):
    ...
    # 3. معالجة فتح قوائم المستوى الأول المباشرة (bsamat, wedding_tissues, abajorat, katb_kitab_box, mirrors, fans, sublimation, clocks, mabakhara)
    # 🔥 ملاحظة: تم إضافة "mabakhara" هنا
    if data in ["bsamat", "wedding_tissues", "abajorat", "katb_kitab_box", "mirrors", "fans", "sublimation", "clocks", "mabakhara"]: 
    ...
    if data.startswith("buy_"):
        ...
        # 🔥 التحقق إذا كان مبخرة (لأنها في conversation handler منفصل)
        if "mabakhara" in data:
             start_mabakhara_purchase(update, context)
             return
...
def main():
    ...
    # 🔥🔥🔥 معالج خاص للمباخر (جديد - تمت الإضافة) 🔥🔥🔥
    mabakhara_handler = ConversationHandler(
        entry_points=[CallbackQueryHandler(start_mabakhara_purchase, pattern='^buy_mabakhara_.*')],
        states={
            GET_MABKHARA_DETAILS: [MessageHandler(Filters.text & ~Filters.command, receive_mabakhara_details_and_finish)],
            GET_PAYMENT_RECEIPT: [
                MessageHandler(Filters.photo, handle_payment_photo),
                CallbackQueryHandler(handle_payment_buttons, pattern='^cancel$') 
            ]
        ...
        fallbacks=[
            CommandHandler('start', start),
            CallbackQueryHandler(back_to_mabakhara_menu, pattern='^mabakhara$'),
            CallbackQueryHandler(cancel_and_end)
        ]
    )
    ...
    dp.add_handler(mabakhara_handler)
